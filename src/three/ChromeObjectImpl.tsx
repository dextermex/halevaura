...
function Studio({ resolution }: { resolution: number }) {
  return (
...
export default function ChromeObject({
  shape = 'sphere', className, spin = 0.06, progress, pointer = false, float = true, zoom = 3.2, style, quality = 'standard',
}: { shape?: Shape; className?: string; spin?: number; progress?: MotionValue<number>; pointer?: boolean; float?: boolean; zoom?: number; style?: React.CSSProperties; quality?: 'standard' | 'high' }) {
      ...
      {ok && (
        ...
        >
          ...
          <Suspense fallback={null}>
            <Studio resolution={quality === 'high' ? 1024 : 512} />
            {float && !prm ? (
